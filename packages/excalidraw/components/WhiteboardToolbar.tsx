...
const getColorOptionStyle = (color: string): React.CSSProperties => ({
  ["--whiteboard-swatch-color" as string]:
          ...
          <div className="whiteboard-toolbar__content">
                ...
                <div className="whiteboard-toolbar__inline-swatches">
                  {INLINE_STROKE_COLORS.map((color) => (
                    <button
                      key={color}
                      type="button"
                      aria-pressed={currentStrokeColor === color}
                      className={clsx(
              ...
              >
                <Popover.Trigger asChild>
                  ...
                  >
                    ...
                    <span className="whiteboard-toolbar__color-label">
                      填充
                    </span>
                  </button>
                </Popover.Trigger>
                    ...
                    <div className="whiteboard-toolbar__color-grid">
                      {BG_COLORS.map((color) => (
                        <button
                          key={color}
                          type="button"
                          aria-pressed={currentBgColor === color}
                          className={clsx("whiteboard-toolbar__color-option", {
                            "whiteboard-toolbar__color-option--active":
                              currentBgColor === color,
                            "whiteboard-toolbar__color-option--transparent":
                              color === "transparent",
                          })}
                          style={getColorOptionStyle(color)}
                          onClick={() => {
            ...
            <div className="whiteboard-toolbar__section whiteboard-toolbar__section--tools">
              {MAIN_TOOLS.map((tool) => (
                <button
                  ...
                  className={clsx("whiteboard-toolbar__tool-button", {
                    "whiteboard-toolbar__tool-button--active":
                      activeTool === tool.type,
                  })}
                  onClick={() => handleToolSelect(tool.type)}
                  title={tool.label}
                >
                  <span className="whiteboard-toolbar__tool-icon">
                    {tool.icon}
                  </span>
                  <span className="whiteboard-toolbar__tool-label">
                    {tool.label}
                  </span>
                </button>
              ))}
              ...
              <DropdownMenu open={isMoreToolsOpen} placement="top">
                <DropdownMenu.Trigger
                  className={clsx(
                    "whiteboard-toolbar__tool-button whiteboard-toolbar__tool-button--more",
                    {
                      "whiteboard-toolbar__tool-button--active":
                        MORE_TOOLS.some((tool) => tool.type === activeTool),
                    },
                  )}
                  onToggle={() => {
            ...
            <div className="whiteboard-toolbar__section whiteboard-toolbar__section--properties">
              ...
              <Popover.Root
                ...
                onOpenChange={setIsStrokeWidthOpen}
              >
                <Popover.Trigger asChild>
                  <button
                    type="button"
              ...
              >
                <Popover.Trigger asChild>
                  <button
                    ...
                    title="透明度"
                  >
                    <span className="whiteboard-toolbar__prop-value">
                      {currentOpacity}%
                    </span>
                    ...
                    <div className="whiteboard-toolbar__prop-options">
                      {OPACITY_VALUES.map((op) => (
                        <button
                          ...
                          type="button"
                          className={clsx(
                            "whiteboard-toolbar__prop-option whiteboard-toolbar__prop-option--text",
                            {
                              "whiteboard-toolbar__prop-option--active":
                                currentOpacity === op.value,
                            },
                          )}
                          onClick={() => {
